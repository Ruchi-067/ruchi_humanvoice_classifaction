...
@st.cache_data
def load_data():
    return pd.read_csv(r"C:\Users\ruchi\OneDrive\Desktop\streamlit_app\env\Scripts\voice_data.csv")

df = load_data()

# Load model and scaler
base_dir = os.path.dirname(__file__)
model_path = os.path.join(base_dir, "voice_gender_classifier_all_features.pkl")
if os.path.isfile(model_path):
    with open(model_path, 'rb') as f:
        model = joblib.load(f)
else:
    model = None
    st.warning(f"Saved model not found: {model_path}. The app will continue without the pre-trained model.")
...
if os.path.isfile(scaler_path):
    ...
    scaler = None
    st.warning(f"Saved scaler not found: {scaler_path}. Uploaded audio will be scaled using a fresh scaler fitted on the dataset.")
...
if menu == "Introduction":
    ...
    st.subheader("🎛️ Upload Audio File for Prediction")
    ...
    if audio_file is not None:
        try:
            ...
            if scaler is None:
                ...
                input_audio = fallback_scaler.transform([features_flat + [0] * (X.shape[1] - len(features_flat))])
            else:
